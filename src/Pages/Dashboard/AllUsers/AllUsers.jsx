...
const AllUsers = () => {
  ...
  const handlerUserLoad = (id) => {
    axiosSecure.get(`/users/specific/${id}`).then((res) => {
    ...
    <>
          ...
          <tbody>
            {allUsers?.map((user, idx) => (
              <tr key={user?._id} className="border-2 border-green-500">
                ...
                <td>
                  <button
                    onClick={() => handlerUserLoad(user?._id)}
                    className="btn btn-sm hover:bg-purple-600 rounded bg-primary text-white"
                  >
                    {" "}
                    <span
                      onClick={() =>
      ...
      <dialog id="my_modal_3" className="modal">
        <div className="modal-box">
          ...
          <h1 className="text-center pt-5 text-2xl text-blue-700">
            {" "}
            Name : {specific?.name}
          </h1>
          ...
          <div className="py-4 pl-2 flex justify-around">
            <button className="btn  bg-indigo-700 hover:bg-success text-white">
              {" "}
              {specific?.role}{" "}
            </button>
            <button className="btn  bg-indigo-700 hover:bg-success text-white">
